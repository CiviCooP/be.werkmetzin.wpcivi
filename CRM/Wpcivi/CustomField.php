 ...
 * @license AGPL-3.0
 */
class CRM_Wpcivi_CustomField {

  protected $_apiParams = array();

  /**
   * CRM_Wpcivi_CustomField constructor.
   */
  ...
  private function validateCreateParams($params) {
    if (!isset($params['name']) || empty($params['name']) || !isset($params['custom_group_id'])
      ...
      mandatory parameters and can not be empty in class CRM_Wpcivi_CustomField'));
    }
  ...
  public function create($params) {
    ...
    if (!isset($this->_apiParams['label']) || empty($this->_apiParams['label'])) {
      $this->_apiParams['label'] = CRM_Wpcivi_Utils::buildLabelFromName($this->_apiParams['name']);
    }
    try {
