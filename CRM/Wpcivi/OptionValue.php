 ...
 * @license AGPL-3.0
 */
class CRM_Wpcivi_OptionValue {

  protected $_apiParams = array();

  /**
   * CRM_Wpcivi_OptionValue constructor.
   */
  ...
  protected function validateCreateParams($params) {
    if (!isset($params['name']) || empty($params['name'])) {
      throw new Exception(ts('Missing mandatory param name in class CRM_Wpcivi_OptionValue'));
    }
    if (!isset($params['option_group_id']) || empty($params['option_group_id'])) {
      throw new Exception(ts('Missing mandatory param option_group_id in class CRM_Wpcivi_OptionValue'));
    }
