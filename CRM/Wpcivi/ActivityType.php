 ...
 * @license AGPL-3.0
 */
class CRM_Wpcivi_ActivityType extends CRM_Wpcivi_OptionValue {
  /**
  ...
  protected function validateCreateParams($params) {
    if (!isset($params['name']) || empty($params['name'])) {
      throw new Exception(ts('Missing mandatory param name in class CRM_Wpcivi_ActivityType'));
    }
    $this->_apiParams = $params;
    try {
      $this->_apiParams['option_group_id'] = $this->getOptionGroupId();
    } catch (CiviCRM_API3_Exception $ex) {
      throw new Exception(ts('Unable to find option group for activity_type in CRM_Wpcivi_ActivityType, contact your system administrator'));
    }
  }
