 ...
 * @license AGPL-3.0
 */
class CRM_Wpcivi_OptionGroup {

  protected $_apiParams = array();

  /**
   * CRM_Wpcivi_OptionGroup constructor.
   */
  ...
  protected function validateCreateParams($params) {
    if (!isset($params['name']) || empty($params['name'])) {
      throw new Exception('Missing mandatory param name in class CRM_Wpcivi_OptionGroup');
    }
  ...
  protected function processOptionValues($optionGroupId, $optionValueParams) {
    foreach ($optionValueParams as $optionValueName => $params) {
      ...
      $optionValue = new CRM_Wpcivi_OptionValue();
